...
public interface BookRepository extends JpaRepository<Book, Long> {
	...
	@Query(value = "update book set available_copies=available_copies+?2 and total_copies=total_copies+?2 where book_id=?1", nativeQuery = true)
	@Transactional
	@Modifying
	void updateExistingCopies(Long bookId, int count);
}
